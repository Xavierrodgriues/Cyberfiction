...
class View4List extends View4
{
    ...
    public $FormName = "fview4list";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "View4List";
    ...
    public function __construct()
    {
        ...
        // Table object (view4)
        if (!isset($GLOBALS["view4"]) || $GLOBALS["view4"]::class == PROJECT_NAMESPACE . "view4") {
            $GLOBALS["view4"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "View4Add";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "View4Delete";
        $this->MultiUpdateUrl = "View4Update";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "View4View"); // If View page, no primary button
                } else { // List page
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview4srch", $filters);
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->phonenum, $default, true); // phonenum
    ...
    public function showFilterList()
    {
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->phonenum, false, true);
        }
        if ($filter != "") {
        ...
        // Field pincode
        $filter = $this->queryBuilderWhere("pincode");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->pincode, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->pincode->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview4list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview4list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "pincode");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview4srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview4srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview4list"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view4", "data-rowtype" => RowType::ADD]);
    ...
    protected function loadSearchValues()
    {
        ...
        // pincode
        if ($this->pincode->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    protected function newRow()
    {
        ...
        $row['pincode'] = $this->pincode->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // phonenum
            if ($this->phonenum->UseFilter && !EmptyValue($this->phonenum->AdvancedSearch->SearchValue)) {
                if (is_array($this->phonenum->AdvancedSearch->SearchValue)) {
                    $this->phonenum->AdvancedSearch->SearchValue = implode(Config("FILTER_OPTION_SEPARATOR"), $this->phonenum->AdvancedSearch->SearchValue);
                }
                $this->phonenum->EditValue = explode(Config("FILTER_OPTION_SEPARATOR"), $this->phonenum->AdvancedSearch->SearchValue);
            }

            // pincode
            $this->pincode->setupEditAttributes();
            $this->pincode->EditValue = $this->pincode->AdvancedSearch->SearchValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview4list" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview4srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
