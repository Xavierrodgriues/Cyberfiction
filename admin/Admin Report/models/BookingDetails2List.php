 ...
 * Page class
 */
class BookingDetails2List extends BookingDetails2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BookingDetails2List";
    ...
    public $FormName = "fbooking_details2list";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "BookingDetails2List";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'booking_details2';
        $this->TableName = 'booking details';
        ...
        // Table object (booking_details2)
        if (!isset($GLOBALS["booking_details2"]) || $GLOBALS["booking_details2"]::class == PROJECT_NAMESPACE . "booking_details2") {
            $GLOBALS["booking_details2"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "BookingDetails2Add";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "BookingDetails2Delete";
        $this->MultiUpdateUrl = "BookingDetails2Update";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'booking details');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "BookingDetails2View"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fbooking_details2grid";
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fbooking_details2srch", $filters);
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->starter_name, $default, false); // starter_name
        $this->buildSearchSql($where, $this->main_course_name, $default, false); // main_course_name
        $this->buildSearchSql($where, $this->sweet_dish_name, $default, false); // sweet_dish_name
    ...
    public function showFilterList()
    {
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->starter_name, false, false);
        }
        if ($filter != "") {
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->main_course_name, false, false);
        }
        if ($filter != "") {
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->sweet_dish_name, false, false);
        }
        if ($filter != "") {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fbooking_details2list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fbooking_details2list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbooking_details2srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbooking_details2srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbooking_details2list"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_booking_details2", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_booking_details2",
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->starter_name->Raw) {
                $this->starter_name->AdvancedSearch->SearchValue = HtmlDecode($this->starter_name->AdvancedSearch->SearchValue);
            }
            $this->starter_name->EditValue = HtmlEncode($this->starter_name->AdvancedSearch->SearchValue);
            ...
            if (!$this->main_course_name->Raw) {
                $this->main_course_name->AdvancedSearch->SearchValue = HtmlDecode($this->main_course_name->AdvancedSearch->SearchValue);
            }
            $this->main_course_name->EditValue = HtmlEncode($this->main_course_name->AdvancedSearch->SearchValue);
            ...
            if (!$this->sweet_dish_name->Raw) {
                $this->sweet_dish_name->AdvancedSearch->SearchValue = HtmlDecode($this->sweet_dish_name->AdvancedSearch->SearchValue);
            }
            $this->sweet_dish_name->EditValue = HtmlEncode($this->sweet_dish_name->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fbooking_details2list" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fbooking_details2srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
