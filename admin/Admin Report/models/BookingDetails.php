...
/**
 * Table class for booking_details
 */
class BookingDetails extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "booking_details";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // room_no
        $this->room_no = new DbField(
            $this, // Table
            'x_room_no', // Variable name
            'room_no', // Name
            '`room_no`', // Expression
            '`room_no`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`room_no`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->room_no->InputTextType = "text";
        $this->room_no->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['room_no'] = &$this->room_no;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "booking_details";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->room_no->DbValue = $row['room_no'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("BookingDetailsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "BookingDetailsView" => $Language->phrase("View"),
            "BookingDetailsEdit" => $Language->phrase("Edit"),
            "BookingDetailsAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "BookingDetailsView",
            Config("API_ADD_ACTION") => "BookingDetailsAdd",
            Config("API_EDIT_ACTION") => "BookingDetailsEdit",
            Config("API_DELETE_ACTION") => "BookingDetailsDelete",
            Config("API_LIST_ACTION") => "BookingDetailsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("BookingDetailsView", $parm);
        } else {
            $url = $this->keyUrl("BookingDetailsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "BookingDetailsAdd?" . $parm;
        } else {
            $url = "BookingDetailsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BookingDetailsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BookingDetailsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BookingDetailsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BookingDetailsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("BookingDetailsDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        // room_no
        $this->room_no->setupEditAttributes();
        if (!$this->room_no->Raw) {
            $this->room_no->CurrentValue = HtmlDecode($this->room_no->CurrentValue);
        }
        $this->room_no->EditValue = $this->room_no->CurrentValue;
        $this->room_no->PlaceHolder = RemoveHtml($this->room_no->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->room_no);
