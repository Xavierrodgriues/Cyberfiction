 ...
 * Page class
 */
class BookingorderReportList extends BookingorderReport
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BookingorderReportList";
    ...
    public $FormName = "fbookingorder_reportlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "BookingorderReportList";
    ...
    public function setVisibility()
    {
        ...
        $this->trans_resp_mesg->setVisibility();
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'bookingorder_report';
        $this->TableName = 'bookingorder report';
        ...
        // Table object (bookingorder_report)
        if (!isset($GLOBALS["bookingorder_report"]) || $GLOBALS["bookingorder_report"]::class == PROJECT_NAMESPACE . "bookingorder_report") {
            $GLOBALS["bookingorder_report"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "BookingorderReportAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "BookingorderReportDelete";
        $this->MultiUpdateUrl = "BookingorderReportUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'bookingorder report');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "BookingorderReportView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fbookingorder_reportgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->trans_resp_mesg->AdvancedSearch->toJson(), ","); // Field trans_resp_mesg
        ...
        $filterList = Concat($filterList, $this->name1->AdvancedSearch->toJson(), ","); // Field name1
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fbookingorder_reportsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field trans_resp_mesg
        $this->trans_resp_mesg->AdvancedSearch->SearchValue = @$filter["x_trans_resp_mesg"];
        $this->trans_resp_mesg->AdvancedSearch->SearchOperator = @$filter["z_trans_resp_mesg"];
        $this->trans_resp_mesg->AdvancedSearch->SearchCondition = @$filter["v_trans_resp_mesg"];
        $this->trans_resp_mesg->AdvancedSearch->SearchValue2 = @$filter["y_trans_resp_mesg"];
        $this->trans_resp_mesg->AdvancedSearch->SearchOperator2 = @$filter["w_trans_resp_mesg"];
        $this->trans_resp_mesg->AdvancedSearch->save();
        ...
        // Field name1
        $this->name1->AdvancedSearch->SearchValue = @$filter["x_name1"];
        $this->name1->AdvancedSearch->SearchOperator = @$filter["z_name1"];
        $this->name1->AdvancedSearch->SearchCondition = @$filter["v_name1"];
        $this->name1->AdvancedSearch->SearchValue2 = @$filter["y_name1"];
        $this->name1->AdvancedSearch->SearchOperator2 = @$filter["w_name1"];
        $this->name1->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->trans_resp_mesg, $default, false); // trans_resp_mesg
        $this->buildSearchSql($where, $this->datentime, $default, false); // datentime
        $this->buildSearchSql($where, $this->name, $default, true); // name
        $this->buildSearchSql($where, $this->name1, $default, false); // name1
        $this->buildSearchSql($where, $this->price, $default, true); // price
        $this->buildSearchSql($where, $this->status, $default, true); // status
        $this->buildSearchSql($where, $this->is_verified, $default, true); // is_verified
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->trans_resp_mesg->AdvancedSearch->save(); // trans_resp_mesg
            $this->datentime->AdvancedSearch->save(); // datentime
            $this->name->AdvancedSearch->save(); // name
            $this->name1->AdvancedSearch->save(); // name1
    ...
    public function showFilterList()
    {
        ...
        // Field trans_resp_mesg
        $filter = $this->queryBuilderWhere("trans_resp_mesg");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->trans_resp_mesg, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->trans_resp_mesg->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field datentime
        $filter = $this->queryBuilderWhere("datentime");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->datentime, false, false);
        }
        if ($filter != "") {
        ...
        // Field name1
        $filter = $this->queryBuilderWhere("name1");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->name1, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->name1->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->is_verified, false, true);
        }
        if ($filter != "") {
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->trans_resp_mesg;
        $searchFlds[] = &$this->name;
        $searchFlds[] = &$this->name1;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->trans_resp_mesg->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->name1->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->trans_resp_mesg->AdvancedSearch->unsetSession();
        $this->datentime->AdvancedSearch->unsetSession();
        $this->name->AdvancedSearch->unsetSession();
        $this->name1->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->trans_resp_mesg->AdvancedSearch->load();
        $this->datentime->AdvancedSearch->load();
        $this->name->AdvancedSearch->load();
        $this->name1->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->trans_resp_mesg); // trans_resp_mesg
            $this->updateSort($this->datentime); // datentime
            $this->updateSort($this->name); // name
            $this->updateSort($this->name1); // name1
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fbookingorder_reportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fbookingorder_reportlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "trans_resp_mesg");
            $this->createColumnOption($option, "datentime");
            $this->createColumnOption($option, "name");
            $this->createColumnOption($option, "name1");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbookingorder_reportsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbookingorder_reportsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbookingorder_reportlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_bookingorder_report", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_bookingorder_report",
    ...
    protected function loadSearchValues()
    {
        ...
        // trans_resp_mesg
        if ($this->trans_resp_mesg->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->trans_resp_mesg->AdvancedSearch->SearchValue != "" || $this->trans_resp_mesg->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // name1
        if ($this->name1->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->name1->AdvancedSearch->SearchValue != "" || $this->name1->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['trans_resp_mesg'] = $this->trans_resp_mesg->DefaultValue;
        $row['datentime'] = $this->datentime->DefaultValue;
        $row['name'] = $this->name->DefaultValue;
        $row['name1'] = $this->name1->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->trans_resp_mesg->Raw) {
                $this->trans_resp_mesg->AdvancedSearch->SearchValue = HtmlDecode($this->trans_resp_mesg->AdvancedSearch->SearchValue);
            }
            $this->trans_resp_mesg->EditValue = HtmlEncode($this->trans_resp_mesg->AdvancedSearch->SearchValue);
            $this->trans_resp_mesg->PlaceHolder = RemoveHtml($this->trans_resp_mesg->caption());

            // datentime
            $this->datentime->setupEditAttributes();
            $this->datentime->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->datentime->AdvancedSearch->SearchValue, $this->datentime->formatPattern()), $this->datentime->formatPattern()));
            ...
            if (!$this->name1->Raw) {
                $this->name1->AdvancedSearch->SearchValue = HtmlDecode($this->name1->AdvancedSearch->SearchValue);
            }
            $this->name1->EditValue = HtmlEncode($this->name1->AdvancedSearch->SearchValue);
            ...
            // is_verified
            if ($this->is_verified->UseFilter && !EmptyValue($this->is_verified->AdvancedSearch->SearchValue)) {
                if (is_array($this->is_verified->AdvancedSearch->SearchValue)) {
                    $this->is_verified->AdvancedSearch->SearchValue = implode(Config("FILTER_OPTION_SEPARATOR"), $this->is_verified->AdvancedSearch->SearchValue);
                }
                $this->is_verified->EditValue = explode(Config("FILTER_OPTION_SEPARATOR"), $this->is_verified->AdvancedSearch->SearchValue);
            }
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fbookingorder_reportsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
