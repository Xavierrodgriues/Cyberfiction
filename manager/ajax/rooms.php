...
if (isset($_POST['add_room'])) {
    // converting data of json in associative array
    ...
    // // room_id which is auto generated will be given by this function
    $room_id = mysqli_insert_id($con);
    $q2 = "INSERT INTO `room_facilities`(`room_id`, `facilities_id`) VALUES (?,?)";
    // // we will not pass the query in the insert() because this query needs to execute many time and so e want it to prepare 
    // // query only one time and if we pass it in the insert() the query will be prepare multiple time which is time consuming
...
if (isset($_POST['get_all_rooms'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        <tr class='align-middle'>
            ...
            <td>$row[quantity]</td>
            <td>$status</td>
            <td><button type='button' onclick='edit_details($row[id])' class='btn btn-primary shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#edit-room'>
...
if (isset($_POST['get_room_images'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        echo<<<data
        <tr class='align-middle'>
            <td><img src='$path$row[image]' style="width:20rem" class='img_fluid'></td>
            <td>$thumb_btn</td>
            <td>
...
//by default the thumb image will set=0
// so the green check box will not appear
